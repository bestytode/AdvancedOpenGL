...
int main()
{
	...
	Shader shader("res/shaders/instantiation.vs", "res/shaders/instantiation.fs");
	...
	while (!glfwWindowShouldClose(window)) {
		...
		GLint location = glGetUniformLocation(shader.GetID(), "offsets");

		GLfloat offsetArray[200];
		for (int i = 0; i < 100; ++i) {
			offsetArray[i * 2] = translations[i].x;
			offsetArray[i * 2 + 1] = translations[i].y;
		}
		glUniform2fv(location, 100, offsetArray);
